...
**Status**: 🔵 Phase 0 - UFC Scaffolding (Ready)
**Priority**: P0 - CRITICAL
**Session Type**: Code Implementation and Remediation Session
**Refactor Target**: `au_sys_unified_storage`
...
- **UNIFIED_STORAGE_ARCHITECTURE_v1.0.0.md** - Service Specification
- **UFC_APP_BLUEPRINT_v1.0.0.md** (v1.1.0) - UFC App Integration Standard
- **UFC_APP_INTEGRATION_ADDENDUM.md** - Detailed Integration Guide (this session)
- **🆕 UFC Capability Factory** - `tooling/au-sys-tools/ufc_capability_factory/` - Software Factory lifecycle automation
...
1.  **Consistency**: Black, Isort
2.  **Correctness**: Mypy (Strict), Ruff (Linting)
3.  **Security**: Bandit (Low/Medium/High)
4.  **Simplicity**: McCabe Complexity < 10
...
**Audit Report**: `au_sys_unified_storage_audit.md`
**Compliance Status**: ❌ **CRITICAL FAILURE**

### Executive Summary
The current `au_sys_unified_storage` codebase represents a **legacy/divergent implementation** that fundamentally violates UFC Architecture Blueprint v1.0.0. It is **effectively unusable** in a UFC system and requires complete refactoring.

### Critical Violations Identified

#### 1. Package Naming Inconsistency (CRITICAL)
- **Issue**: Directory name `au_sys_unified_storage` ≠ pyproject.toml name `au_sys_storage`
- **Impact**: BREAKS BUILD - violates "Identity of Structure" mandate
- **Resolution**: Standardize on `au_sys_unified_storage` throughout

#### 2. Missing Core Layer - Layer 1 (CRITICAL)
- **Required**: `core/` with `models/`, `schemas/`, `services/`, `ports/`
- **Actual**: **COMPLETELY MISSING**
- **Impact**: No Domain Layer - business logic scattered or absent
- **Resolution**: Create complete Core layer structure + migrate logic

#### 3. Missing Manifest Layer - Layer 3 (CRITICAL)
- **Required**: `manifest/` with `plugin.py`, `container.py`, `config.py`
- **Actual**: **MISSING** - only static `plugin.yaml` exists (BANNED)
- **Impact**: No DI Container, no dynamic plugin loading
- **Resolution**: Implement Manifest layer + remove static YAML

#### 4. Missing Scaffold Module (HIGH)
- **Required**: `scaffold/` for self-extraction capability
- **Actual**: **MISSING**
- **Impact**: Cannot participate in UFC self-scaffolding pattern
- **Resolution**: Create Scaffold module with template extraction logic

#### 5. Missing Scripts Directory (HIGH)
- **Required**: `scripts/` with `dev/`, `build/`, `bootstrap/`, `ops/`
- **Actual**: **MISSING**
- **Impact**: No lifecycle management automation
- **Resolution**: Implement complete scripts structure with automation tools

#### 6. Non-Compliant Adapters Layer - Layer 2 (MEDIUM)
- **Required**: `api`, `web`, `mcp`, `cli` (standard types)
- **Actual**: Only `admin` and `api` exist
- **Issues**: `admin` is non-standard; missing `web`, `mcp`, `cli`
- **Resolution**: Restructure adapters + migrate `admin` logic to `web/admin/`

### Compliance Scorecard

| UFC Component | Required | Present | Status | Priority |
|--------------|----------|---------|--------|----------|
| Package Naming Consistency | ✅ | ❌ | FAIL | P0 |
| Core Layer (Layer 1) | ✅ | ❌ | FAIL | P0 |
| Adapters Layer (Layer 2) | ✅ | ⚠️ | PARTIAL | P1 |
| Manifest Layer (Layer 3) | ✅ | ❌ | FAIL | P0 |
| Scaffold Module | ✅ | ❌ | FAIL | P1 |
| Scripts Directory | ✅ | ❌ | FAIL | P1 |

**Overall Compliance**: 0% (0/6 mandatory components fully compliant)

### Audit Verdict
**Status**: NON-COMPLIANT
**Action Required**: Complete "Greenfield" refactor to UFC v1.0.0
**Migration Path**: NOT salvageable through incremental fixes - requires systematic rebuild
...
### Primary Goal
Execute the **end-to-end Codebase Lifecycle Protocol** with **"Near Full Automation,"** strictly prioritizing the use of existing and new scripts over manual intervention.

### Automation Strategy
Success is defined by the **minimal amount of manual editing**. The process must be driven by the `scripts/dev/` automation suite.

### Target Package
**Package**: `au_sys_unified_storage`
**Location**: `libraries/python/services/au_sys_unified_storage`
**Source Root**: `src/au_sys_unified_storage`

### Legacy Migration Mapping

| Source (legacy_source) | Target (src/au_sys_unified_storage) | Description |
|------------------------|-------------------------------------|-------------|
